...
class Settings(BaseSettings):
    ...
    nuxt_auth_token: str = Field(
        default="insecure-default-nuxt-token-for-local-development-only-change-in-prod",
        env="NUXT_AUTH_TOKEN",
        description="Секретный токен, который должен"
                    " приходить от Nuxt для подтверждения запроса"
    )
